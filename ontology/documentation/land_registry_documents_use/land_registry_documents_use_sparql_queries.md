...
WHERE {
    ...
    #Folio details
    ?plot addr:hasAttribute/addr:hasAttributeVersion/cad:isMentionnedIn/rico:isOrWasConstituentOf+ ?folio.
...
WHERE {
    ...
    #Folio details
    ?taxpayer addr:hasTrace ?taxpayerversion.
...
WHERE {
    ...
    ?classement cad:mentions/addr:isAttributeVersionOf/addr:isAttributeOf/addr:isTraceOf ?plot.
    ?plot dcterms:identifier ?id.
} 
```
...
WHERE {
    ...
    ?mutation addr:hasAttribute/addr:hasAttributeVersion/cad:hasTaxpayer/addr:isTraceOf ?taxpayer.
    ?taxpayer rdfs:label ?label.
} 
```
...
where {
    ...
    ?plot addr:hasAttribute/addr:hasAttributeVersion/cad:takenFrom cad_spval:ResteSV.
} 
```